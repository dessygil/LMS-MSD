...
async function handler(req, res) {
  try {
    const accessTokenRequest = await axios.post(`${process.env.AUTH0_ISSUER_BASE_URL}/oauth/token`, {
    ...
    const privateApiEndpointResponse  = await axios.get("http://api.localhost:8000/api/private", {
      headers: {
